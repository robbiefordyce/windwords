...
class Bible(KingJames1611):
    def __init__(self):
        ...
        self.scripture_re = re.compile(
            self._scripture_regex(), re.IGNORECASE | re.UNICODE
        )

    def _scripture_regex(self):
        return (
            r'\b(?P<BookTitle>%s)\s*'
            r'(?:%s|\s)*'
            ...
            r')?'
        ) % (self.book_re_string, self.stopwords_regex(), self.stopwords_regex(), self.stopwords_regex(), self.stopwords_regex())
    ...
    def stopwords_regex(self):
        ...
        stopwords.extend(["chapter", "chapters", "verse", "verses"])
        return "|".join(sorted(stopwords, key=lambda s: len(s), reverse=True))
